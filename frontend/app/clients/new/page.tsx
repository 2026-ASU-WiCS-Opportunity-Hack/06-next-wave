    ...
    const load = async () => {
      ...
      if (eventId) {
        const { data: ev } = await supabase
          ...
          .single()
        setEvent(ev)
        const typeName = (ev?.service_types as any)?.name ?? ''
        const { getFormSchema } = await import('@/lib/formTemplates')
const fields = await getFormSchema(
  (ev?.service_types as any)?.id,
  typeName,
  supabase
)
setDynamicFields(fields)
        if (ev?.service_types) {
          setServiceEntry(prev => ({
            ...prev,
            service_type_id: (ev.service_types as any).id
          }))
        }
      }
    }
            ...
            {dynamicFields.map((field: DynamicField) => (
              <div key={field.key}>
                <label className={labelClass}>
                  {t(field.key, lang)}{field.required && ' *'}
                </label>
                {field.type === 'boolean' ? (
                  <select
